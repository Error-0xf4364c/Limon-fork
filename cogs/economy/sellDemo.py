...
class MyButtons(View):
    ...
    async def sellfishes_callback(self, interaction, button):
        ...
        await interaction.response.edit_message(content= f"🐟 **|** Balıklarınızı başarıyla sattınız. Toplam geliriniz **{sum_fish}** Cupcoin",view=self)
    ...
    async def sellhunts_callback(self, interaction, button):
        ...
        await coincollection.replace_one({"_id": interaction.user.id}, user_data_coins)
        await collection.replace_one({"_id": interaction.user.id}, user_data)
        await interaction.response.edit_message(content=f"🦌 **|** Avladığınız hayvanları başarıyla sattınız. Toplam geliriniz **{sum_hunt}** Cupcoin", view=self)
